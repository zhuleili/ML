...
def getOnes(index, nowtime):
    arr = nowtime.split('-')
    week = datetime.datetime(int(arr[0]), int(arr[1]), int(arr[2])).strftime("%w")
    if week == '0':
        week = '7'
    ...
    res = ''
    try:
        req = urllib.request.Request(url='http://www.okooo.com/jingcai/?action=more&LotteryNo=%s&MatchOrder=%s%s' % (nowtime, week, index), headers=header_dict)
...
def getAddress(nowTime):
    ...
    try:
        ...
        while xulie != -1:
            ...
            if html.find('count') != -1:
                start = html.find('class="p1 count">') + 17
            else:
                start = html.find('class="p1">') + 11
            end = html.find('</p>')
            soure = html[start:end]  # 比赛结果
            # html = html[end + 5:]
            count = 0
            data = getOnes(index, nowTime)  # 获取详细信息
            while count < 10:
                if data != '':
                    break
                else:
                    count += 1
                    logger.info('%s  %s比赛数据获取失败，重新获取第%s次' % (nowTime, index, count))
                    time.sleep(1)
                    data = getOnes(index, nowTime)  # 获取详细信息
            ...
            if float(sheng) <= float(fu):  # 胜/胜（A）和平/胜(B)的赔率，计算一下（A*B）/（A+B）
                if float(sheng) == 0:
                    ...
                    if float(sheng) - 1.00 == 0:
                        valuse.append('0.00')
                    else:
                        valuse.append('%.2f' % ((jisuan - 1.00)/(float(sheng) - 1.00)))
                ...
                if float(fu) - 1.00 == 0:
                    ...
                    valuse.append('%.2f' % ((jisuan - 1.00) / (float(fu) - 1.00)))
            valuse.append(soure)  # 加入比赛结果
            result[result.index(index)] = valuse

            logger.info('%s  %s比赛数据已获取' % (nowTime, index))
            xulie = html.find('xulie_h')
...
def createTable(nowTime):
        if not os.path.exists('D:/投注信息/月/%s-%s.xls' % (nowTime[0], nowTime[1])):
            mkdir('D:/投注信息/月/')
            workbook = xlsxwriter.Workbook('D:/投注信息/月/%s-%s.xls' % (nowTime[0], nowTime[1]))
            for i in range(int(nowTime[2])):
                workbook.add_worksheet(str(i + 1))
            workbook.close()


def insertExcel(result, nowTime):
    rb = open_workbook('D:/投注信息/月/%s.xls' % nowTime[0:7])
    wb = copy(rb)
    ws = wb.get_sheet(str(int(nowTime[8:10])))
    ws.write(0, 0, '编号')
    ws.write(0, 1, '胜')
    ws.write(0, 2, '平')
    ws.write(0, 3, '负')
    ...
    wb.save('D:/投注信息/月/%s.xls' % nowTime[0:7])
...
if __name__ == '__main__':
    ...
    nowTime = time.strftime("%Y-%m-%d", time.localtime()).split('-')
    createTable(nowTime)
    for i in range(int(nowTime[2])):
        if i + 1 < 10:
            i = '0%s' % str(i + 1)
            ...
            i = str(i + 1)
        mouth('%s-%s-%s' % (nowTime[0], nowTime[1], i))
    logger.info('全部完成')

...
def createTable(info,nowTime):
        if not os.path.exists('D:/投注信息/14家/%s.xls' % nowTime):
            mkdir('D:/投注信息/14家/')
            workbook = xlsxwriter.Workbook('D:/投注信息/14家/%s.xls' % nowTime)
            for i in info:
...
def get_row(nowTime):
    rb = open_workbook('D:/投注信息/14家/%s.xls' % nowTime)
    ...
    try:
        ...
        for i in range(len(url_arr)):
            ...
            if res.find('data-pname') != -1:
                ...
                pass
        if len(fail) > 0:
            logger.info('=====完成,失败场次：%s 文件路径:D:/投注信息/14家/%s.xls)=====' % (fail, nowTime))
            logger.info('')
        else:
            logger.info('=====全部完成,文件路径:D:/投注信息/14家/%s.xls)=====' % nowTime)
            logger.info('')
    except Exception as e:
...
def insertExcel(sheet, url, nowTime, result, count):
        ...
        for j in range(len(result)):
            for k in range(len(result[j])):
                if k == 1:
                    ...
                    arr4.append(float(result[j][k]))
                elif k == 5:
                    arr5.append(float(result[j][k]))
                elif k == 6:
                    arr6.append(float(result[j][k]))
                elif k == 7:
                    arr7.append(float(result[j][k]))
                elif k == 8:
                    arr8.append(float(result[j][k]))
                elif k == 9:
                    arr9.append(float(result[j][k]))
                elif k == 10:
                    arr10.append(float(result[j][k]))
                ws.write(j + count + 2, k, result[j][k])
    ws.write(count + 16, 1, '%.2f' % float(sum(arr1) / len(arr1)))
    ws.write(count + 16, 2, '%.2f' % float(sum(arr2) / len(arr2)))
    ws.write(count + 16, 3, '%.2f' % float(sum(arr3) / len(arr3)))
    ws.write(count + 16, 4, '%.2f' % float(sum(arr4) / len(arr4)))
    ws.write(count + 16, 5, '%.2f' % float(sum(arr5) / len(arr5)))
    ws.write(count + 16, 6, '%.2f' % float(sum(arr6) / len(arr6)))
    ws.write(count + 16, 7, '%.2f' % float(sum(arr7) / len(arr7)))
    ws.write(count + 16, 8, '%.2f' % float(sum(arr8) / len(arr8)))
    ws.write(count + 16, 9, '%.2f' % float(sum(arr9) / len(arr9)))
    ws.write(count + 16, 10, '%.2f' % float(sum(arr10) / len(arr10)))
    ...
    ws.write(count + 17, 4, '%.2f' % re1 + '%')
    ws.write(count + 17, 5, '%.2f' % re2 + '%')
    ws.write(count + 17, 6, '%.2f' % re3 + '%')
    wb.save('D:/投注信息/14家/%s.xls' % nowTime)

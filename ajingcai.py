...
def getResult(url_arr, info):
    ...
    for i in range(len(url_arr)):
        ...
        res = getOldOnes(url_arr[i])

        result = []
        name_start = res.find('data-pname')
        count = 1
        while name_start != -1 and count < 20:
            count += 1
            data = []
            res = res[name_start + 12:]  # 名字开始的位置
            name_end = res.find('">')  # 用来记录名字结束的位置
            name = res[0: name_end]
            if name == '99家平均' or name == '必发' or name == '竞彩官方':
                data.append(name)
...
def getOldOnes(arr):
    ...
    try:
        ...
        req = urllib.request.Request(url='%sajax/?companytype=BaijiaBooks&page=0&trnum=30&type=0' % arr, headers=header_dict)
        res = urllib.request.urlopen(req)
        res = res.read().decode()
    except Exception as e:
        logger.info(e)
...
def insertExcel(nowTime, result, cell):
    ...
    for i in result:
        ...
        if cell == 0:
            ...
            ws.write(0, cell + 6, '必发')
            ws.write(0, cell + 7, '竞猜官方')
            ...
            ws.write(0, cell + 4, '必发')
            ws.write(0, cell + 5, '竞猜官方')

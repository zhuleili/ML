...
def createTable(info):
        ...
        if not os.path.exists('D:/投注信息/5家/%s(5).xls' % nowFormatTime):
            mkdir('D:/投注信息/5家/')
            workbook = xlwt.Workbook('D:/投注信息/5家/%s(5).xls' % nowFormatTime)
            for i in info:
                wh = workbook.add_sheet(i)
                wh.write(0, 10, '*', get_style(2))
                workbook.save('D:/投注信息/5家/%s(5).xls' % nowFormatTime)
...
def get_row(nowTime):
    try:
        rb = open_workbook('D:/投注信息/5家/%s(5).xls' % nowTime, formatting_info=True)
        ...
        if str(e).find('list index out of range') != -1:
            return 0
        if str(e).find('formatting_info') != -1:
            return 0
    try:
        for i in range(24):
            # val = table.cell(4, row).value
            val = table.row(row)[0].value
            row += 8
    except Exception as e:
        if str(e).find('list index out of range') != -1:
            return row + 1
    ...
    try:
        ...
        for i in range(len(url_arr)):
            ...
            if res.find('data-pname') != -1:
                ...
                insertExcel(i, url_arr[i], nowTime, result, count, wb)
                ...
                pass
        if len(fail) > 0:
            logger.info('=====完成,失败场次：%s 文件路径:D:/投注信息/5家/%s(5).xls)=====' % (fail, nowTime))
            logger.info('')
        else:
            logger.info('=====全部完成,文件路径:D:/投注信息/5家/%s(5).xls)=====' % nowTime)
            logger.info('')
    except Exception as e:
...
def getOnes(arr):
    header_dict = {'Accept': 'text/html, */*',
                   ...
                   'Accept-Language': 'zh-CN,zh;q=0.8',
                   'Connection': 'keep-alive',
                   'Cookie': 'FirstOKURL=http%3A//www.okooo.com/jingcai/; First_Source=www.okooo.com; bookMakerCustomData=a%3A1%3A%7Bs%3A14%3A%22AuthoriteBooks%22%3Ba%3A2%3A%7Bi%3A0%3Ba%3A12%3A%7Bi%3A0%3Bi%3A14%3Bi%3A1%3Bi%3A82%3Bi%3A2%3Bi%3A27%3Bi%3A3%3Bi%3A94%3Bi%3A4%3Bi%3A84%3Bi%3A5%3Bi%3A24%3Bi%3A6%3Bi%3A2%3Bi%3A7%3Bi%3A15%3Bi%3A8%3Bi%3A43%3Bi%3A9%3Bi%3A65%3Bi%3A10%3Bi%3A36%3Bi%3A11%3Bi%3A37%3B%7Ds%3A12%3A%22bookTypeName%22%3Bs%3A12%3A%22%C8%A8%CD%FE%B2%A9%B2%CA%B9%AB%CB%BE%22%3B%7D%7D; Last_Source=http%3A//www.okooo.com/jingcai/; PHPSESSID=a6e491c49ba5c83288da6eb3edda0a366013aa35; DRUPAL_LOGGED_IN=Y; IMUserID=23960264; IMUserName=zll735508686; OkAutoUuid=9f0c4882c8db1421dc83a7b62371442e; OkMsIndex=2; isInvitePurview=0; UWord=5c0ef7418f470ef8ab80183d01d55e128eb; LastUrl=; Hm_lvt_5ffc07c2ca2eda4cc1c4d8e50804c94b=1513660142; Hm_lpvt_5ffc07c2ca2eda4cc1c4d8e50804c94b=1513660315; __utma=56961525.1965621462.1513660140.1513660140.1513660140.1; __utmb=56961525.15.8.1513660315284; __utmc=56961525; __utmz=56961525.1513660140.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none)',
                   'Host': 'www.okooo.com',
                   'Referer': arr,
                   'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36)',
                   'X-Requested-With': 'XMLHttpRequest'
    ...
    res = ''
    try:
        req = urllib.request.Request(url='%sajax/?companytype=AuthoriteBooks&page=0&trnum=30&type=0' % arr, headers=header_dict)
...
def get_style(type):
    font0 = xlwt.Font()
    font0.name = '5'
    font0.colour_index = type  # 2红 3绿
    font0.bold = True
    style0 = xlwt.XFStyle()
    style0.font = font0
    return style0

def insertExcel(sheet, url, nowTime, result, count, wb):
    ...
    arr1_max = arr1.index(max(arr1))
    arr1_min = arr1.index(min(arr1))
    ws.write(count + 2 + arr1_max, 4, str(arr1[arr1_max]), get_style(2))
    ws.write(count + 2 + arr1_min, 4, str(arr1[arr1_min]), get_style(3))
    arr2_max = arr2.index(max(arr2))
    arr2_min = arr2.index(min(arr2))
    ws.write(count + 2 + arr2_max, 5, str(arr2[arr2_max]), get_style(2))
    ws.write(count + 2 + arr2_min, 5, str(arr2[arr2_min]), get_style(3))
    arr3_max = arr3.index(max(arr3))
    arr3_min = arr3.index(min(arr3))
    ws.write(count + 2 + arr3_max, 6, str(arr3[arr3_max]), get_style(2))
    ws.write(count + 2 + arr3_min, 6, str(arr3[arr3_min]), get_style(3))
    wb.save('D:/投注信息/5家/%s(5).xls' % nowTime)
